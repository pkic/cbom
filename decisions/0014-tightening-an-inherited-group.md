# 0014. A derived profile may tighten one member of an inherited group, and widen its key coverage
...
- **Restate the group.** A restated group replaces the inherited one, and with it the base's
  `keyCoverage`. The property most worth inheriting — that a deferred purpose still owes a status,
...
- **Option C — allow key coverage to be widened** without touching members (trade-off: solves breadth
...
`keyCoverage`, which may be widened and never narrowed. Narrowing it is how a staged profile becomes
...
key coverage to its own purposes, every sector would quietly opt out of the questions it found
...
- `load_profile` resolves overrides against inherited group shells and members before concatenating.
  `check_guard_tightens` and `check_key_coverage_tightens` are new; `check_override_tightens` calls the
  first.
...
- The third-level test fixture now tightens `pqc-migration#G1.3` from MAY to MUST and removes its
  ...
  added where the base has none, a guard narrowed, and key coverage narrowed.
...
- Decision 0010, which created the group rule and the key-coverage guarantee this protects; 0013, whose
