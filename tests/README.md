...
and for the data files that drive the site's generated pages.

## Running them

```bash
bash tests/run-profile-tests.sh    # profiles, CBOMs, validator
python3 tests/check-site-data.py   # meetings and presentations data
```
...
Exit status is 0 when every test passes. CI runs both on any change under
`docs/methodology/`, `docs/_data/` or `tests/`, so a broken example or a broken
download link fails visibly rather than sitting on the published site.
...
|---|---|
| A presentation has exactly one of `file:` and `url:` | Neither means there is nothing to download; both means the page silently prefers one. |
| A `file:` is actually committed under `docs/assets/presentations/` | Otherwise the download button 404s. |
| A presentation's `meeting:` matches a real meeting number | Otherwise the cross-link between the two pages goes nowhere. |
| Meeting numbers and presentation ids are unique | Both are linked to from the other page. |
...
| Profile well-formedness | All three example profiles satisfy C1 to C17, including under `--strict`. One fixture per MUST requirement confirms that each is enforced and that the right requirement is the one reported. |
...
| Entry profile | The shallowest profile of the interface family is well-formed under C1 to C17 and `--strict`, and the documents sort into three groups: what a scanner emits with no profile in mind conforms to neither depth; `cbom-fail` and `cbom-disclosure`, both rejected by the baseline, conform to the entry profile; the two that conform to the baseline conform to both. A shared rule is cited against the tag of whichever profile imposed it, which is decision 0011 being visible in a report rather than argued for. |
| The family is one ladder | `check-family.py`. A family ordered by depth claims that conformance at a depth carries conformance below it, and the two profiles are siblings rather than parent and child, so nothing structural enforces it (Q51). The check requires that the deeper profile drops no rule, that shared rules are identical, that the vocabularies and identifier schemes they resolve against are identical &mdash; `enumRef` resolves per profile, so the same rule text can permit different values at two depths &mdash; that the disclosure convention matches, that scope and carrier range do not widen, and that every committed document conforming to the baseline conforms to the entry profile. It also fails if no document sits between the two depths, since a family whose rungs nothing distinguishes has one rung. Checked against four deliberate divergences: an extra rule at the shallow depth, a widened vocabulary, a raised carrier minimum, and a changed marker prefix. |
